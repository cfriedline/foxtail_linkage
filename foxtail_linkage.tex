...
used a simple case of a single maternal tree with two chromosomes each of which contains 100 loci.
...
probability of $0.10$ that one of the known haploid genotypes is changed to the alternate allele. After
introduction of errors in this fashion, missing data were simulated by randomly removing data for each
of the 76 haplotypes at random using the level of missing data observbed for each haplotype in the red library
as the per loci probability of being missing. For example, if an observed haplotype has $2\%$ missing data
...
estimated. All simulations were done using custom scripts in R \citep{R:2013}.
