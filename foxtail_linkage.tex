...
the optimal assembly for SNP calling. This resulted in multiple sets of SNPs for which we quantified various aspects: number of SNPs, amount of 
missing data, and overlap with the optimal assembly (see Consensus Map 
...
call the maternal genotype to filter SNPs prior to linkage mapping. (MORE HERE).

Lastly, we explored the thresholds used in the linkage mapping. The relevant thresholds were
those used in imputation and error correction. We varied the threshold upon which
we allowed the maximum about of data, as being more stringent with taking only those
samples with the most data may affect the uniqueness or non-uniqueness of the
minimum spanning tree used during order. We also varied the threshold determining the number
of consecutive loci allowed to have incorrect genotypes from XXXX.

\subsection*{Consensus Map Construction and Biological Interpretation}

The four linkage maps, one for each maternal tree, were combined into a consensus map using the MergeMap \citep{Wu:2008b}. Prior to building the consensus linkage map, contigs
shared by each map were identified through use of XXXXX.
