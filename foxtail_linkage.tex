...
the maternal tree had to be a heterozygote for us to detect a SNP, and Mendel's first law 
guarantees that the segregation ratio should thus be 1:1. Second, we filtered the resulting SNPs based on the genotype of the pseudo-diploid sample so as to keep only those SNPs
...
reference assembly. The assembly process, however, requires optimization of the hash size (see DNA Sequence Analysis),
...
Construction and Biological Interpretation). When loci were shared across SNP sets, we conducted linkage mapping as
described previously for each set and compared the resulting maps of the shared loci using Spearman rank correlations \citep{Spearman:1904}.

Second, we explored the use of thresholds for segregation distortion, amount of missing data, and 
use of the pseudo-diploid sample to call the maternal genotype to filter SNPs prior to linkage mapping.

Lastly, we explored the thresholds used in the linkage mapping. We varied the threshold upon which
...
examined using as thresholds, 1\%, 5\%, and 10\% of the number of loci per cluster.

\subsection*{Consensus Map Construction and Biological Interpretation}

The four linkage maps, one for each maternal tree, were combined into a consensus map using MergeMap \citep{Wu:2008b}. Prior to building the consensus linkage map, contigs that were
