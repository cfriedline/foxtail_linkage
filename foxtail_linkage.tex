...
The individual with the largest number of reads was assembled using 
...
script (for odd $k$ on $k=[19,65]$).  Assembly robustness was evaluated using the LAP likelihood 
...
likelihood estimate. The assembly with the highest average maximum likelihood value was 
chosen as the reference for SNP calling (Figure \ref{f:mapping_performance}).

SNPs were called for all individuals against the reference using a variety of methods.  First, 
...
we excluded SNPs using a $\chi^2$ test against an expectation of 1:1 segregation. This segregation 
...
for quality reduced these numbers by approximately XX-fold, with a range 
of a XX-fold (XXXX) to a XX-fold (XXXXX) reduction. After filtering, the average
...
across libraries, with minimums of approximately XX to maximums of approximately XX. After 
quality filtering this translated into an average total of XX bp generated per library, with XX to XX bp 
per megagametophyte per library.  

The largest number of reads (n = XXXX) were obtained for a single megagmetophyte in the green library.
These reads were used to create an assembly for which alll other data were mapped against for SNP calling and genotype determination.
Optimization of assembly parameters (list here, see Figure 2) resulted in an assembly with 231,053 contigs, with an average length
of 90 bp per contig (range: \SIrange {61}{312}{bp}). {STATEMENT HERE ABOUT COVERAGE FOR EACH CONTIG}
...
Using this assembly, 349,542 putative SNPs were called (Table 1). These 349,542 SNPs were located in
83,051 unique contigs (35.94\% of the total), with a mean of 4 SNPs per contig (range: \SIrange{1}{32}{}). Filtering these SNPs by
expected segregation patterns, consistency with heterozygous calls for the psuedo-diploid sample, and minimum
sample sizes for genotype calls, resulted in 983, 34,261, 21,594, and 35,304 SNPs for the red, green, blue, 
...
recombination fractions. These SNP counts represented 557, 17,051, 11,883, and 17,506 contigs
...
On average, megagametophytes in the filtered dataset had XX\% missing data across SNPs (range: 1.5\% to 99.\%), 
...
\begin{figure}[ht]
\centering
\subfigure[Network of $n=30$ nodes]{\includegraphics[width=0.45\textwidth]{g1}}
\subfigure[Strongly connected subgraphs ($n=21$) of (A)]{\includegraphics[width=0.45\textwidth]{g2}}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{scc_subgraph}
\caption{Example subgraph of $n=207$ strongly connected components for SNPs shared across all libraries.  
Each shape indicates a unique contig and the direction is oriented according the aligment 
which connectes the contig from query (dotted) to target (solid).}
\label{f:scc_subgraph}
\end{figure}
