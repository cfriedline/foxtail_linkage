...
assessed using a Bonferroni-corrected $\alpha = 0.05$, where $\alpha$ was corrected using the number of SNPs tested. 
...
reference assembly. The assembly process, however, requires optimization of the hash size (see \textbf{DNA Sequence Analysis}),
...
the optimal assembly for SNP calling. When loci were shared across SNP sets (see \textbf{Consensus Map 
...
analysis ($n = 10,000$ permutations), using a Bonferroni-corrected $\alpha = 0.05$. Non-significant values 
...
was tested using permutation analysis ($n = 10,000$ permutations) and a Bonferroni-corrected $\alpha = 0.05$. 
Non-significant values were taken as evidence of significantly different orderings.
In all cases, linkage mapping as described previously  (i.e., used all of the other previous thresholds unless otherwise noted) 
...
using a Bonferroni-corrected $\alpha = 0.05$. Non-significant values were taken as evidence of significantly different orderings.
