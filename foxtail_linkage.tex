...
of substitutions between two sequences \citep[2\textit{ut}, see][]{Nei:1987}, where the mutation rate (\textit{u}) was assumed to be $1.0$ x $10^{-9}$ substitutions/site/year and the
divergence time (\textit{t}) was assumed to be $8.0$ x $10^{7}$ years \citep{Willyard:2007}. This translated into an average expectation of 16\% divergence
...
around this expectation. Hits that exceeded these thresholds were transferred as annotations to the contig appearing on the 
...
different geographical regions. The resulting linkage maps for each maternal tree also resulted in SNPs from the same contig being mapped to the same position, with an average of only XX\% of SNPs from the same contig being mapped to a different position. For those SNPs from the same contig that
